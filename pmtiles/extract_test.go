...
func TestReencodeEntries(t *testing.T) {
	...
	entries = append(entries, EntryV3{1, 500, 20, 0})

	reencoded, result := ReencodeEntries(entries)

	assert.Equal(t, 2, len(result))
	assert.Equal(t, result[0].Offset, uint64(400))
	assert.Equal(t, result[0].Length, uint64(10))
	assert.Equal(t, result[1].Offset, uint64(500))
...
func TestReencodeEntriesDuplicate(t *testing.T) {
	...
	entries = append(entries, EntryV3{0, 400, 10, 0})
	entries = append(entries, EntryV3{1, 500, 20, 0})
	entries = append(entries, EntryV3{2, 400, 10, 0})
...
func TestReencodeContiguous(t *testing.T) {
	...
	_, result := ReencodeEntries(entries)
...
func TestMergeRanges(t *testing.T) {
	ranges := make([]Range, 0)
	ranges = append(ranges, Range{0, 50})
	ranges = append(ranges, Range{60, 60})

	result := MergeRanges(ranges, 0.1)

	assert.Equal(t, 1, len(result))
	assert.Equal(t, Range{0, 120}, result[0].Rng)
...
func TestMergeRangesMultiple(t *testing.T) {
	ranges := make([]Range, 0)
	ranges = append(ranges, Range{0, 50})
	ranges = append(ranges, Range{60, 10})
	ranges = append(ranges, Range{80, 10})

	result := MergeRanges(ranges, 0.3)
	assert.Equal(t, 1, len(result))
	assert.Equal(t, Range{0, 90}, result[0].Rng)
