...
func Show(_ *log.Logger, bucketURL string, key string, showMetadataOnly bool, showTilejson bool, publicURL string, showTile bool, z int, x int, y int) error {
	...
	if !showTile {
		...
		if showMetadataOnly {
			...
			fmt.Printf("bounds: %f,%f %f,%f\n", float64(header.MinLatE7)/10000000, float64(header.MinLonE7)/10000000, float64(header.MaxLatE7)/10000000, float64(header.MaxLonE7)/10000000)
			fmt.Printf("min zoom: %d\n", header.MinZoom)
			fmt.Printf("max zoom: %d\n", header.MaxZoom)
			fmt.Printf("center: %f,%f\n", float64(header.CenterLatE7)/10000000, float64(header.CenterLonE7)/10000000)
