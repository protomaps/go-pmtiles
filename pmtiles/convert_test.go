...
func TestV2UpgradeExtra(t *testing.T) {
	...
	_, ok := json_metadata["center"]
	assert.False(t, ok)
	_, ok = json_metadata["abc"]
	assert.True(t, ok)
}
...
func TestMbtiles(t *testing.T) {
	...
	_, ok = json_metadata["name"]
	assert.True(t, ok)

	_, ok = json_metadata["format"]
	assert.True(t, ok)

	_, ok = json_metadata["attribution"]
	assert.True(t, ok)

	_, ok = json_metadata["description"]
	assert.True(t, ok)

	_, ok = json_metadata["type"]
	assert.True(t, ok)

	_, ok = json_metadata["version"]
	assert.True(t, ok)

	_, ok = json_metadata["compression"]
	assert.True(t, ok)

	// assert well-known json fields at top level
	_, ok = json_metadata["vector_layers"]
	assert.True(t, ok)
