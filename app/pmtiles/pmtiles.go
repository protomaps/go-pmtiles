...
func Run(logger *log.Logger) error {
	...
	case "convert <input> <output>":
		...
		if cli.Convert.Tmpdir == "" {
			...
			if err != nil {
				return fmt.Errorf("Failed to derive absolute path for %s, %v", cli.Convert.Tmpdir, err)
			}
	...
	case "verify <input>":
		...
		if err != nil {
			return fmt.Errorf("Failed to verify archive, %v", err)
		}
	case "version":
